...
public class AccountConcurrentLinkedQueue implements Account {
	...
	private AtomicReference<Operation> atomicTotal = new AtomicReference<>(new Operation(0, 0, 0, DEPOSIT_OPERATION, 0));
	...
	public void deposit(long amount) {		
		...
		do {
			...
			newValue = new Operation(newOrder, new Date().getTime(), newTotalAmount, DEPOSIT_OPERATION, amount);
	...
	public boolean withdraw(long amount) {
		...
		do {
			...
			// check that the value has not been changed since the last time we read it
	...
	/**
	 * Another implementation of the withdraw method.
	 * In this implementation, all operations are logged even when there aren't enough money to complete the operation
	 */
	public boolean withdrawWithAdditionalOperationType(long amount) {
		...
		do {
			...
			newValue =  new Operation(error ? prev.getOrder() : prev.getOrder() + 1, new Date().getTime(), error ? prev.getTotalAmount() - amount : prev.getTotalAmount(), 
					error ? WIDTHDRAW_OPERATION : FAILED_WIDTHDRAW_OPERATION, amount);			
			if(atomicTotal.compareAndSet(prev, newValue)){
				break;
			}
		} while (true);	
